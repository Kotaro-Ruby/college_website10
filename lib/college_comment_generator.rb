module CollegeCommentGenerator
  def self.generate_comment_for_college(college_name, college_data = {})
    # 大学の種類を判定
    college_type = determine_college_type(college_name, college_data)
    ...
    # ベースコメントを選択
    base_comment = comment_templates[college_type].sample
    
    # 大学の特徴に基づいてコメントを拡張
    if college_data[:students] && college_data[:students] > 0
      if college_data[:students] > 30000
        base_comment += " 大規模な学生数を抱える総合大学として、多様な学習機会と活発なキャンパスライフが魅力です。"
      elsif college_data[:students] > 10000
        base_comment += " 中規模の大学として、適度な規模感の中で充実した教育とサポートを受けることができます。"
      else
        base_comment += " 小規模な環境を活かし、教授との距離が近く、きめ細かい指導を受けることができます。"
      end
    end
    
    if college_data[:acceptance_rate] && college_data[:acceptance_rate] > 0
      if college_data[:acceptance_rate] < 0.3
        base_comment += " 高い選考基準で知られ、優秀な学生が集まる環境で切磋琢磨できます。"
      elsif college_data[:acceptance_rate] > 0.8
        base_comment += " 多くの学生に学習機会を提供することを重視し、幅広い背景の学生を受け入れています。"
      end
    end
  ...
  def self.determine_college_type(college_name, college_data)
    name_lower = college_name.downcase
    
    # 2年制大学
    return 'community_college' if college_data[:school_type]&.include?('2-year') || 
