namespace :import do
  desc "Import colleges from specific states batch"
  task :states_batch, [:start_index, :end_index] => :environment do |t, args|
    require 'net/http'
    require 'json'
    require 'dotenv/load'
    ...
    # 全米50州のリスト
    all_states = ['CA', 'TX', 'FL', 'NY', 'PA', 'IL', 'OH', 'GA', 'NC', 'MI', 
                  'NJ', 'VA', 'WA', 'AZ', 'MA', 'TN', 'IN', 'MO', 'MD', 'WI', 
                  'CO', 'MN', 'SC', 'AL', 'LA', 'KY', 'OR', 'OK', 'CT', 'UT',
                  'IA', 'NV', 'AR', 'MS', 'KS', 'NM', 'NE', 'ID', 'WV', 'HI',
                  'NH', 'ME', 'RI', 'MT', 'DE', 'SD', 'ND', 'AK', 'VT', 'WY']
    
    start_idx = (args[:start_index] || 0).to_i
    end_idx = (args[:end_index] || 9).to_i
    
    states = all_states[start_idx..end_idx]
    
    puts "インポート対象州: #{states.join(', ')}"
