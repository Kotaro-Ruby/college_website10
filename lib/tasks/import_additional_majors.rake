namespace :college_data do
  desc "Import additional major fields from College Scorecard API"
  task import_additional_majors: :environment do
    require 'net/http'
    require 'json'
    require 'uri'
    ...
    # 追加の専攻分野のマッピング（College Scorecard APIのフィールド名）
    additional_major_fields = {
      'agriculture' => 'pcip_agriculture',                        # 農業・農学
      'natural_resources' => 'pcip_natural_resources',           # 天然資源・環境科学
      'communications' => 'pcip_communication',                  # コミュニケーション学
      'foreign_language' => 'pcip_foreign_languages',            # 外国語・文学
      'english' => 'pcip_english',                              # 英語・文学
      'mathematics' => 'pcip_mathematics',                       # 数学・統計学
      'social_science' => 'pcip_social_sciences',               # 社会科学
      'visual_performing' => 'pcip_visual_arts',                # 視覚・舞台芸術
      'history' => 'pcip_history',                              # 歴史学
      'philosophy_religious' => 'pcip_philosophy',              # 哲学・宗教学
      'physical_sciences' => 'pcip_physical_sciences',          # 物理科学
      'legal' => 'pcip_law'                                     # 法学・刑事司法
    }

    def fetch_schools_additional(api_key, page = 0, per_page = 100)
      uri = URI('https://api.data.gov/ed/collegescorecard/v1/schools.json')
      
      # 追加の専攻分野をリクエスト
      program_fields = [
        'latest.academics.program_percentage.agriculture',
        'latest.academics.program_percentage.natural_resources',
        'latest.academics.program_percentage.communications',
        'latest.academics.program_percentage.foreign_language',
        ...
        'latest.academics.program_percentage.physical_sciences',
    ...
    puts "追加専攻データのインポートを開始します..."
    
    loop do
      puts "ページ #{page + 1} を処理中..."
      
      data = fetch_schools_additional(api_key, page, per_page)
      ...
      schools.each do |school|
        ...
        if condition
          ...
          # 各追加専攻分野のデータを取得
          additional_major_fields.each do |api_field, db_field|
